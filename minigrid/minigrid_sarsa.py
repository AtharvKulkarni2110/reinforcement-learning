...
for epi in range(num_episodes):                                    
    ...
    while not done:                                    #agent takes actions till terminal state is achieved or episode is truncated midway.
        ...
        Q[state][action]+=alpha*(reward+gamma*(Q[next_state][A1])-Q[state][action])     #as q learning is on-line update algo we update Q value after each action with the help of previous q value of state and behaviour policy on next state which acts always greedily 
    ...
    epsilon = max(0.01, epsilon * 0.96)              #epsilon decay
